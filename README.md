...
- [ ] **Milestone 5 — Workouts + History screens.** Still stubbed: `WorkoutsScreen`'s field callbacks and Save button are `TODO`, `WorkoutsViewModel` has no `save()`/`loadForEdit(id)` logic yet, and `HistoryScreen`'s delete action is a `TODO`. Wire these up, then smoke test: log a workout, see it in History, delete it, re-add and edit it.
